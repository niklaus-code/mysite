...
- ceph daemon /var/run/ceph/6ff02758-4907-11ec-bd8f-525400420c5f/ceph-osd.0.asok dump_historic_ops

- ceph daemon /var/run/ceph/6ff02758-4907-11ec-bd8f-525400420c5f/ceph-osd.0.asok  perf dump

```
    select u.username, u.company, t.name as bucket, date_format(t.ctime, '%Y-%m-%d %h:%i:%s') 
    as ctime, t.rdonly, t.id as bkid, t.userid, false origin  
    from obs_buckets t 
    left join auth_user u on t.userid=u.id 
    union select u.username, u.company, s.name as bucket, date_format(o.sharetime, '%Y-%m-%d %h:%i:%s') 
     as ctime, o.readonly, o.bkid,  u.id, true origin    
    from obs_buckets_shared o 
    left join auth_user u on o.email=u.email 
    left join obs_buckets s on o.bkid=s.id
    order by ctime 
```

- ceph-s 没反映可能是mon节点问题重启或者停调mon节点
